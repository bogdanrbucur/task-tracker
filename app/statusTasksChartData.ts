...
export default function statusTasks(tasks: TaskExtended[]) {
	...
	tasks.forEach((task) => {
		...
		if (!statuses.some((s) => s.name === task.status.name))
			statuses.push({ name: task.status.name, value: 1, url: `/tasks?status=${task.status.id}`, slug: task.status.name.replaceAll(" ", "").toLowerCase() });
		// If the status is already in the array, increment the value
		else statuses.find((s) => s.name === task.status.name)!.value++;
	});
