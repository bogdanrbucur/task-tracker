	...
	if (!userStats) {
		await prisma.userStats.create({
			data: {
				...
				totalDaysWorkingOnTasks: taskAction === "complete" ? differenceInCalendarDays(today, task.completedOn!) | 0 : 0,
		...
		await prisma.userStats.update({
			...
			data: {
				...
				totalDaysWorkingOnTasks:
					taskAction === "complete" ? userStats.totalDaysWorkingOnTasks! + (differenceInCalendarDays(today, task.completedOn!) | 0) : userStats.totalDaysWorkingOnTasks,
